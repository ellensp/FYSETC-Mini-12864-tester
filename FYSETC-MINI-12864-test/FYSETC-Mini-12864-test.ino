...
//Lets you test the standard features of a FYSETC-Mini-128x64 LCD
//Power on tests the RGB LED's  First is Red, Second is Green, Third is Blue and finally White (all on)  
...
void setup() {
  ...
  //Show Red, Green, Blue leds, then use white.
  delay(1000);
  ...
  delay(1000);
  set_neopixel_color(pixels.Color(255, 255, 255, 0)); // white
}
...
void draw(){
  u8g.setColorIndex(0);
  u8g.drawBox(0,0,screenX,screenY);
  u8g.setColorIndex(1);

  u8g.drawStr( 2+x, 10, "FYSETC Mini TEST");
  ...
  u8g.drawFrame(0,0,screenX,screenY);
